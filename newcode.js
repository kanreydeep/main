...
async function login(clientId) {
    if (DEBUG_MODE) {
        return `${APP_TOKEN}:deviceid:${clientId}:${Date.now()}`;
    }
...
function generateKeys() {
    const selectedApp = document.getElementById('gameSelect').value;
    updateConstants(selectedApp);

    const numKeys = parseInt(document.getElementById('keyCountSelect').value, 10);
    findValidKeys(numKeys);
}

// Event listener for the start button
document.getElementById('startBtn').addEventListener('click', generateKeys);